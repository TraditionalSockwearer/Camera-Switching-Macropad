...
async def main(): 
    keyboard._init(hid_type=HIDModes.USB)
    
    # Task 1: Keyboard Loop (Fast)
    async def keyboard_handler():
        while True:
            keyboard._main_loop()
            await asyncio.sleep(0)
    
    # Task 2: Movie Extension Loop (Animation/Ping/Scroll)
    asyncio.create_task(movie_ext.loop())
    
    await keyboard_handler()
